    ...
    selectedPowerMode(0)//ultimo meto herdado 
{
    ...
    RedefinirPesquisa();//reseta o slave 
...
uint8_t DStemperatura:: SelecionarEndereco(uint8_t address[]) {
...
String DStemperatura::ObterNumerodeDespositos(){
  return  String(QtdDespositivos);
}
uint8_t DStemperatura::SelecionaProxSensor(){
    if (oneWireSearch(SEARCH_ROM)) {
        return  SelecionarEndereco(searchAddress);
    }

    return 0;
}

uint8_t DStemperatura:: SelecionarProxAlarme() {
...
void DStemperatura::RedefinirPesquisa() {
    lastDiscrepancy = 0;
    lastDevice = 0;
}
...
float DStemperatura::ObterTemperaturaCelcius() {
  ...
  TempDeConversao(selectedResolution, selectedPowerMode);
    readScratchpad();
...
void DStemperatura::SetarResolucaes(uint8_t resolucao) {
...
void DStemperatura::ObterEndereco(uint8_t address[]) {
...
this->SetarResolucaes(oldResolution);
...
uint8_t DStemperatura::readScratchpad() {
   this->sendCommand(MATCH_ROM, READ_SCRATCHPAD);
...
void DStemperatura::writeScratchpad() {
  ...
  umfio.write(selectedScratchpad[ALARM_HIGH]);
   ...
   this-> sendCommand(MATCH_ROM, COPY_SCRATCHPAD, !selectedPowerMode);
...
void DStemperatura::TempDeConversao(uint8_t resolucao, uint8_t powerMode) {
    if (powerMode) {
